...
>: View {
    ...
    /// Optional string rendered inside the drag pill, to the right of the
    /// drag indicator and any leading/trailing accessory buttons. Used by
    /// doit to show a live token count in SF Mono with an animate-up
    /// transition. When `nil` or empty, no extra label is rendered.
    var handleTrailingText: String? = nil

    /// Cap on the title width inside the collapsed (minimal) pill. Without
    /// this, a long todo title forces the capsule to grow past the screen,
    /// dragging the rest of the layout sideways. With it, the title
    /// truncates with an ellipsis and the capsule maxes out at this width.
        ...
        ZStack {
            ...
            HStack(spacing: 8) {
                ...
                Text(isMinimalPill ? (hideTop ? topTitle : bottomTitle) : "")
                    ...
                    .lineLimit(1)
                    .truncationMode(.tail)
                    .frame(maxWidth: minimalPillTextMaxWidth)
            ...
            HStack(spacing: 8) {
                ...
                // Invisible width-anchor that mirrors the outer pill's
                // sizing Text (line ~311). Only rendered when the pill
                // collapses to its minimal capsule — there it forces the
                // capsule to wrap the title. When not minimal, leaving it
                // empty avoids long titles ballooning the inner HStack
                // (and therefore the entire ZStack) wider than the screen,
                // which otherwise drags the top/bottom views off-canvas.
                Text(isMinimalPill ? (hideTop ? topTitle : bottomTitle) : "")
                    .lineLimit(1)
                    .truncationMode(.tail)
                    .frame(maxWidth: minimalPillTextMaxWidth)
                ...
                HStack(spacing: 8) {
                    if let label = handleTrailingText,
                       !label.isEmpty,
                       !isMinimalPill,
                       !isAccessoriesPill {
                        Text(label)
            ...
            ZStack {
                ...
                // Mirrors the outer pill text — only shown (and taking
                // layout space) in minimal mode. A long title with
                // `.fixedSize()` here otherwise widens the parent ZStack
                // beyond the screen even though the Text is invisible.
                Text(isMinimalPill ? (hideTop ? topTitle : bottomTitle) : "")
                    .fontWeight(.medium)
                    .lineLimit(1)
                    .truncationMode(.tail)
...
/// Pulls a single numeric magnitude out of a formatted string like
/// `"12.3K tok"` so SwiftUI's `numericText(value:)` content transition can
/// pick a direction (up vs down) when the label changes. Falls back to 0
/// when the string contains no leading digits.
fileprivate func numericValue(of text: String) -> Double {
    var digits = ""
    for ch in text {
        if ch == "," { continue }
        if ch.isNumber || ch == "." {
    ...
    guard let base = Double(digits) else { return 0 }
    let upper = text.uppercased()
    if upper.contains("B") { return base * 1_000_000_000 }
    if upper.contains("M") { return base * 1_000_000 }
    if upper.contains("K") { return base * 1_000 }
    return base
}
